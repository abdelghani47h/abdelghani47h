<div align="center" width="100%">
  <img src="https://github.com/SP-XD/SP-XD/blob/main/images/dev-working_rounded.gif?raw=true" alt="Workspace" width="40%"/><br>
</div>

<hr>

### 👋 Hello! I'm **Abdelghani AL Ghoul** <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="30">
A recent Computer Science graduate 🚀. I'm passionate about software development, especially **Desktop, Web & .NET Applications**.  
I have a solid foundation in **C, C++, C#**, and **SQL**, and I enjoy building projects that reflect my learning journey 💻.  
Also, I am exploring **Cloud Computing** with **AWS** and **Azure**, aiming to combine programming and cloud expertise.

---

### 🛠️ Skills & Tools
<div align="center">
  <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" />
  <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white" />
  <img src="https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dot-net&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-003B57?style=for-the-badge&logo=Microsoft-SQL-Server&logoColor=white" />
  <img src="https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white" />
  <img src="https://img.shields.io/badge/Azure-0078D4?style=for-the-badge&logo=microsoft-azure&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Visual%20Studio-5C2D91?style=for-the-badge&logo=visual-studio&logoColor=white" />
</div>

---

### 💻 Featured Projects
- [<span style="color: #1E90FF;">Project 1 – Stone Paper Scissor</span>](https://github.com/abdelghani47h/RockPaperScissors) 🛠️ A simple game built with C++ that demonstrates logic and problem-solving skills.
- [<span style="color: #1E90FF;">Project 2 – Math Quiz Game</span>](https://github.com/abdelghani47h/MathQuizGame) ⚡ A C++ console application that generates random arithmetic questions.

---

### 🏆 Achievements & Certifications
- Completed **14+ courses in Abu-Hadhoud Roadmap (Programming Advices)**  
- Solved **200+ programming problems**  
- Completed multiple courses in **C, C++, C#, .NET**, and **SQL**  
- **AWS Certified Cloud Practitioner**  
- **AWS AI Practitioner**  
- **Microsoft Azure Fundamentals (AZ-900)**  
- Built personal projects to reinforce programming skills

---

### 🎯 Currently Working On
- **Abu-Hadhoud Roadmap**  
- Mastering **C# & OOP 💡**  
- Mastering **Desktop, Web & .NET Applications**  
- Strengthening problem-solving and algorithms with **C++ & C**  
- Exploring **Cloud Computing** with AWS & Azure, preparing cloud projects  
- Preparing .NET Web projects for GitHub portfolio  
- Continuously improving **GitHub & LinkedIn profiles** for professional presence

<div align="center">

  <!-- GitHub Stats -->
  <img src="https://github-readme-stats.vercel.app/api?username=abdelghani47h&show_icons=true&theme=tokyonight" alt="GitHub Stats" />

  <br><br>

  <!-- Top Languages -->
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=abdelghani47h&layout=compact&theme=tokyonight&langs_count=5" alt="Top Languages" />

  <br><br>

  <!-- Followers Badge -->
  <img src="https://img.shields.io/github/followers/abdelghani47h?label=Followers&style=for-the-badge&logo=github&color=1f6feb" alt="GitHub Followers" />

</div>


### 📬 Contact Information
<div align="center">
  <a href="https://www.linkedin.com/in/abdelghani47h/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:abdelghanialghoul@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</div>

---

```cpp
// Mastering Desktop, Web & Cloud Development
class AbdelghaniALGhoul {
public:
  string degree = "Computer Science";
  string focus = "Desktop, Web & Cloud Developer";
  string skills = "C, C++, C#, .NET, SQL, AWS & Azure";
  void sayHi() { cout << "Thanks for visiting my profile! 🚀"; }
};
